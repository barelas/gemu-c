/* simulation of Least Work First (LWF) scheduling algorithm */
...
void schedule() /*simple LWF scheduling*/
{
	...
	/* select job with least workload */
	while (j) {
		if ( (j->state == WAITING) && (j->workload < best_job->workload) )
			best_job = j;
		j = j->next;
	}
